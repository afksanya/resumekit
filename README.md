...
- Choose from `classic`, `ats`, `modern`, `compact`, `sidebar`, and `executive` templates for HTML and PDF
...
Templates can be `classic`, `ats`, `modern`, `compact`, `sidebar`, or `executive`.
...
- 创建命名简历版本
- 导出 HTML、PDF 或 JSON
- 支持 `classic`、`ats`、`modern`、`compact`、`sidebar`、`executive` 六种 HTML 和 PDF 模板
- 记录求职投递
- 查看投递状态摘要
- 校验简历完整度
- 对比两个简历版本
...
模板可以选 `classic`、`ats`、`modern`、`compact`、`sidebar` 或 `executive`。
